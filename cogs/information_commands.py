...
class InformationCommands(vbu.Cog[types.Bot]):
        ...
        application_command_meta=commands.ApplicationCommandMeta(
            options=[
                discord.ApplicationCommandOption(
                    name="plant_name",
                    description="The name of the plant that you want to see the information for.",
    ...
    async def herbiary(self, ctx: vbu.Context, *, plant_name: Optional[str] = None):
        """
        ...
        with vbu.Embed(use_random_colour=True) as embed:
            ...
            ctx.bot.set_footer_from_config(embed)
        display_vbu: PlantDisplayUtils = self.bot.get_cog("PlantDisplayUtils")  # type: ignore
        ...
        gif_frames: List[Image.Image] = [display_vbu.get_plant_image(plant.name, i, "clay", pot_hue) for i in display_levels]
        plant_image_bytes: io.BytesIO = display_vbu.gif_to_bytes(*gif_frames, duration=1_000)

        # And send image
        await ctx.send(
            embed=embed,
            file=discord.File(plant_image_bytes, filename="plant.gif"),
        )
